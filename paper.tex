...
\begin{equation}
	\mathcal N_b = \{(r,\beta) \ldots \} = \{ (r_{b'}, \beta_{b'}) \ldots \} = \{ b' \ldots \}
	\label{notation}
\end{equation}
...
normalised vectors $\hat o$ by $O_b$.  Note that $v^\wedge$ is the equivalent
...
\begin{eqnarray}\label{eq:Obstacle2}
  v_o(b) & = & O_b \hat q_o \\
	...
	\nonumber \\
	v_o(b) & = & O_b \left(\sum_{o\in \mathcal O_b } \hat
	o\right)^{\!\wedge} \nonumber
\end{eqnarray}
