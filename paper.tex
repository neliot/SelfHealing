...
A swarm's perimeter can be used to modify its behaviour, such as reducing sensor requirements by reducing the need for GPS sensors and coordinating a swarm to a destination. When discussing \textit{concave reduction} the angle of interest is the relationship between an agent and two of its adjacent neighbours such that the neighbours have no visibility of each other and the \textit{neighbour-agent-neighbour} relationship produces an angle~$< 180^\circ$. This is unrelated to the aggregate angle used to identify the perimeter type that an agent is a member of i.e. inner or outer perimeter. Concave reduction therefore affects both inner~(Fig.~\ref{fig:InnerPerimeter1}) and outer~(Fig.~\ref{fig:OuterPerimeter1}) perimeter agents. Figures~\ref{fig:OuterPerimeter1} and \ref{fig:InnerPerimeter1} show candidate agents, in black, identified by this process. 
...
The centroid $D_{pos}(b)$ is then used to calculate the concave reduction vector as shown in Equation~\ref{eq:ConcaveVoidPhysics3}.
...
$D(b)$~is a vector derived from the identified centroid of the neighbour gap~($G_b$) to the parent agent~($b$). This new vector is the \textit{concave vector} used to implement the concave reduction movement~(Equation~\ref{eq:ConcaveVoidPhysics3a}).

\begin{equation}\label{eq:ConcaveVoidPhysics3a}
D(b) = bD_{pos}(b) 
\end{equation}

This derived \textit{movement-vector} reduces the anomaly. The concave reduction movement needs to be modified to accommodate an agent's surroundings. Due to agent proximity it can be assumed that there are no agents in the path between the agent and the gap, however, there may be an obstacle in the agent's path. The \textit{concave reduction vector} must include an obstacle avoidance component~$v_o(b)$. As with all the previous vector based calculations a weighting is applied $k_{cr}$ to allow the model to vary the intensity of the effects. The resultant vector is normalised to produce the \textit{concave reduction vector} as shown in~Equation~\ref{eq:ConcaveVoidPhysics4}. 
