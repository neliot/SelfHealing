...
To describe the removal of voids a swarming model needs to be defined. The model use is the same as that defined by Eliot et. al. in their paper that introduces a new magnitude based metric~\cite{EKB:18}. Equation~\ref{eq:BotPhysics1} defines a weighted model that includes cohesion, repulsion, direction and obstacle avoidance ($v_c(b), v_r(b), v_d(b), v_o(b)$). The weightings $k_c, k_r, k_d, k_o$ allow each component to be scaled to tailor the swarming effect. 
